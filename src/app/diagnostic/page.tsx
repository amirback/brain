...
import { subjectById, topicsOf } from "@/lib/content";
import { formatForecast, nextDiagnosticTopic, nextTarget, pickQuestion, readiness } from "@/lib/engine";
...
export default function DiagnosticPage() {
  ...
  const { user, ready, role, recordAnswer, finishDiagnostic, unlock } = useStore();
  ...
  const subject = user?.activeSubject ?? "math";
  const topics = topicsOf(subject);

  // Pick the next question whenever the slot is empty during a run.
  useEffect(() => {
  ...
  const onAnswered = (correct: boolean) => {
    ...
    recordAnswer({
      ...
      correct, difficulty: current.difficulty, mode: "diagnostic",
    });
  ...
  const view = useMemo(
    () => (user ? formatForecast(readiness(user, subject), user.goal) : null),
    [user, subject]
  );

  if (!ready || !user || !view) return null;

  const subjectName = pick(subjectById(subject)?.title ?? { ru: "", kk: "", en: "" });
  const forecastLabel =
    user.goal === "sat" ? "SAT" : user.goal === "ielts" ? "IELTS" : d.diag.resultForecast;
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-lg px-4 py-16 text-center">
          <Reveal>
            ...
            <p className="mt-2 text-[13px] font-semibold uppercase tracking-wider text-brand">{subjectName}</p>
            ...
            <div className="mx-auto mt-7 grid max-w-xs grid-cols-3 gap-2">
              {topics.map((t) => (
                <div key={t.id} className="rounded-2xl border border-line bg-card p-3">
                  ...
                  <div className="mt-1 text-[10px] tabular-nums text-dim">{Math.round(t.weight * 100)}%</div>
                </div>
              ))}
            </div>
            <Btn onClick={() => setPhase("run")} size="lg" className="arrow-slide mt-8">
              {d.diag.begin}
              <span className="arr">
                <IconArrow size={18} />
              </span>
        ...
        <Reveal delay={140}>
          <Card className="h-full">
            ...
            <div className="font-display mt-2 flex items-end gap-2 text-5xl font-extrabold tabular-nums">
              <CountUp to={view.numeric} decimals={user.goal === "ielts" ? 1 : 0} />
              <span className="mb-1.5 text-xl text-dim">/ {view.max}</span>
            </div>
            <div className="mt-1 text-[12.5px] text-dim">{subjectName}</div>
          </Card>
        </Reveal>
      </div>
